  ...
  useEffect(() => {
    ...
    const fetchStudents = async () => {
      try {
        ...
        const response = await axios.get("https://api.karpagamalumni.in/api/students/", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://api.karpagamalumni.in/api/students/",
        formDataToSend,
        {
          headers: {
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
        {students.map((student) => (
          <div key={student.id} className="bg-white rounded-lg shadow p-4">
            {student.image ? (
              <img
                src={`https://api.karpagamalumni.in/api${student.image}`}
                alt={student.name}
