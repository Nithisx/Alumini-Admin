...
export default function Events() {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch("https://0c41dd49b6b06c17e9bfea0690ee4051.serveo.net/api/events/", {
      headers: { Authorization: token ? `Token ${token}` : "" },
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this event?")) {
      fetch(`https://0c41dd49b6b06c17e9bfea0690ee4051.serveo.net/api/events/${id}`, {
        method: "DELETE",
        headers: { 
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((event) => {
              ...
              const imgUrl = imgPath ? `https://0c41dd49b6b06c17e9bfea0690ee4051.serveo.net/api${imgPath}` : null;
              return (
                <div
                  key={event.id}
