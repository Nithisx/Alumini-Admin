...
const Birthday = () => {
  ...
  useEffect(() => {
    const fetchBirthdays = async () => {
      try {
        setLoading(true);
        const response = await fetch('https://0c41dd49b6b06c17e9bfea0690ee4051.serveo.net/api/birthdays/', {
  ...
  const BirthdayCard = ({ user, showDaysUntil = false }) => (
    ...
    >
      <div className="flex-shrink-0">
        {user.profile_photo ? (
          <img
            src={`https://0c41dd49b6b06c17e9bfea0690ee4051.serveo.net/api${user.profile_photo}`}
            alt={user.username}
