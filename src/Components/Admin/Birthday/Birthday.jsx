...
const Birthday = () => {
  ...
  useEffect(() => {
    const fetchBirthdays = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://209.38.121.118/api/birthdays/', {
  ...
  const BirthdayCard = ({ user, showDaysUntil = false }) => (
    ...
    >
      <div className="flex-shrink-0">
        {user.profile_photo ? (
          <img
            src={`http://209.38.121.118/api${user.profile_photo}`}
            alt={user.username}
  ...
  return (
    <div className="max-w-5xl my-10 mx-[20rem] w-full p-6  bg-gray-50 min-h-screen">
