...
const SendMail = () => {
  ...
  const fetchEmailSuggestions = async (query) => {
    ...
    try {
      const response = await axios.get(`https://api.karpagamalumni.in/api/v1/email-suggestions/?query=${query}`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("https://api.karpagamalumni.in/api/v1/send-email/", data, {
        headers: {
          Authorization: `Token ${localStorage.getItem("Token")}`,
