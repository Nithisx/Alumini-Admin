 ...
 *  - Hidden on /chat and /map pages (fixed, full-viewport layouts)
...
function buildCrumbs(pathname, dynamicLabels) {
  ...
  // Always anchor with Dashboard — routes are prefix-free (no role root segment).
  const crumbs = [{ label: "Dashboard", path: "/dashboard", isHome: true }];
...
export default function Breadcrumb() {
  ...
  // Hidden on chat and map — both are fixed, full-viewport pages with their
  // own header, so a breadcrumb strip above them would eat into that space.
  if (location.pathname.includes("/chat") || location.pathname.includes("/map")) return null;
